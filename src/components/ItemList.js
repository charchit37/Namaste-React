...
const ItemList = (items) => {
    return (
        <div>
            {items.item.map((item) => {
                return (
                    <div key={item.card.info.id} className="flex items-center item-container border-b mb-4">
                        <div className="w-3/5">
                            ...
                            <div className="flex items-center gap-1 font-bold text-sm">
                                {item.card.info.ratings.aggregatedRating.rating && (
                                    <>
                                        ...
                                        <div className="rating-star-color">{item.card.info.ratings.aggregatedRating.rating}</div>
                                        <div className="rating-count">{item.card.info.ratings.aggregatedRating.ratingCountV2}</div>
                                    </>
                                )}
                            </div>
                        ...
                        <div className="w-2/5 shrink-0">
                            <div>
                                <button className="rounded-lg bg-white shadow-lg add-btn">
                                    ADD
                                </button>
                                <img  src={CDN_URL + item.card.info.imageId} className="res-img" />
                            </div>
                        </div>
                    </div>
                );
